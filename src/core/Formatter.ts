...
export default class Formatter {
  ...
  formatLineComment(token: IToken, query: string) {
    return this.addNewline(query + token.value);
  }

  formatBlockComment(token: IToken, query: string) {
  ...
  formatToplevelReservedWord(token: IToken, query: string) {
  ...
  formatNewlineReservedWord(token: IToken, query: string) {
    return (
  ...
  formatOpeningParentheses(token: IToken, query: string) {
  ...
  formatClosingParentheses(token: IToken, query: string) {
    if (this.inlineBlock.isActive()) {
  ...
  formatPlaceholder(token: IToken, query: string) {
    return query + this.params.get(token) + ' ';
  }

  // Commas start a new line (unless within inline parentheses or SQL "LIMIT" clause)
  formatComma(token: IToken, query: string) {
  ...
  formatWithSpaceAfter(token: IToken, query: string) {
    return this.trimTrailingWhitespace(query) + token.value + ' ';
  }

  formatWithoutSpaces(token: IToken, query: string) {
    return this.trimTrailingWhitespace(query) + token.value;
  }

  formatWithSpaces(token: IToken, query: string) {
    return query + token.value + ' ';
  }

  formatQuerySeparator(token: IToken, query: string) {
    return this.trimTrailingWhitespace(query) + token.value + '\n';
  }
